...
export function useAccommodationImageGallery({
  ...
  const handleTouchEnd = useCallback(() => {
    if (touchStart == null || touchEnd == null || imageCount <= 0) return;

    const distance = touchStart - touchEnd;
    if (distance > minSwipeDistance && mobileSlideIndex < maxImageIndex) {
      setMobileSlideIndex(mobileSlideIndex + 1);
    }
    if (distance < -minSwipeDistance && mobileSlideIndex > 0) {
      setMobileSlideIndex(mobileSlideIndex - 1);
    }
  }, [
    imageCount,
