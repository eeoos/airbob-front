...
export interface SerializablePublicRuntimeConfig {
  readonly apiBaseUrl: string;
  readonly googleMapsBrowserKey: string | null;
  readonly tossClientKey: string | null;
  readonly cloudFrontHost: string;
...
): SerializablePublicRuntimeConfig => ({
  apiBaseUrl: config.apiBaseUrl,
  googleMapsBrowserKey: config.googleMapsBrowserKey,
  tossClientKey: config.tossClientKey,
  cloudFrontHost: config.cloudFrontHost,
});

export const serializePublicRuntimeConfig = (
  config: PublicRuntimeConfig = getPublicRuntimeConfig(),
): string =>
  JSON.stringify(toSerializablePublicRuntimeConfig(config))
    .replace(/</g, "\\u003c")
    .replace(/\u2028/g, "\\u2028")
    .replace(/\u2029/g, "\\u2029");
...
export const getTossClientKey = (): string | null =>
  getPublicRuntimeConfig().tossClientKey;

export const getCloudFrontDomain = (): string =>
  getPublicRuntimeConfig().cloudFrontHost;

const requireConfiguredValue = (value: string | null, key: string): string => {
  if (!value) {
