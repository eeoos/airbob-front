...
import * as searchBarUrlSyncModule from "./useSearchBarUrlSync";
...
describe("useSearchBarState URL sync boundary", () => {
  beforeEach(() => {
  ...
  it("delegates URL synchronization to a dedicated hook", async () => {
    const urlSyncHook = jest.spyOn(
      searchBarUrlSyncModule,
      "useSearchBarUrlSync",
    );

    renderHook(() => useSearchBarState());

    await waitFor(() => expect(urlSyncHook).toHaveBeenCalled());
  });
});
