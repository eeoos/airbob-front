...
import * as searchBarGuestsModule from "./useSearchBarGuests";
...
describe("useSearchBarState guest boundary", () => {
  beforeEach(() => {
  ...
  it("delegates guest state and guest actions to a dedicated hook", async () => {
    const guestHook = jest.spyOn(
      searchBarGuestsModule,
      "useSearchBarGuests",
    );

    renderHook(() => useSearchBarState());

    await waitFor(() => expect(guestHook).toHaveBeenCalled());
  });
});
