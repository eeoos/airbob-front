...
describe("accommodation read query keys", () => {
  it("scopes viewer-dependent detail reads for anonymous and authenticated viewers", () => {
    ...
    expect(
      accommodationReadQueryKeys.detail(authenticatedScope, 31),
    ).toEqual([
      "accommodation",
      "detail",
      31,
  ...
  it("changes detail identity when the subject or epoch changes", () => {
    ...
    const nextSubject = accommodationReadQueryKeys.detail(
      {
        ...authenticatedScope,
        subject:
          "subject:member_8" as AuthenticatedSessionScope["subject"],
      },
      31,
    );
    ...
    expect(base).not.toContainEqual(expect.objectContaining({ authRefreshIndex: expect.anything() }));
  });
