...
This means route and layer ownership converged, while payment semantics did not. Future work must land one contract, read boundary or user-flow owner at a time. Inert adapters/schema/read-side work can land independently; only the final writer composition switch must be atomic.
...
| U3 should be one production mega-unit                                 | Conflicts with churn evidence and small-commit requirement                                | Split capabilities/read-side; atomic owner switch only                     |
