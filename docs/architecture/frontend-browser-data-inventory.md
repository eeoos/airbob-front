...
> Baseline: U6 routing followed by the U5 session, U19 structural UI, U7 auth/wishlist, U8 Search, and U9 Detail/Review cutovers; recorded 2026-08-30 KST.
...
| `history.state` reservation checkout handoff | Same `ReservationCheckoutState` fields described below | Primary same-navigation handoff to confirm screen | Personal | U9 Detail app route writes through `reservationCheckoutState.ts`; no stable viewer owner exists in the record | Runtime type guard only; history lifetime | The reservation workflow captures and rechecks route/session before one synchronous storage/navigation commit. Confirm still prefers valid location state, but browser/user ownership is not encoded. U5 replaces the current transaction entry during an identity boundary and cannot erase older Back-stack entries. | U10 replaces this compatibility handoff with a minimal subject-owned, one-shot document, rejects foreign/expired/consumed entries before rendering or I/O, and omits personal identity fields unless the gateway demonstrably needs them. This remains a release gate. |
...
U8 switches Search and U9 switches Detail/Review; U10-U13 switch the remaining
legacy parsers/controllers. Those later workflow units remain responsible for
server/session authority. A session cleanup can
...
approval of the unversioned checkout/payment storage/history shapes; U10 still
owns their subject envelope, one-shot reader, schema, TTL, server verification, and
...
| Route/session change during Search/wishlist/detail/review/editor/payment | U5 remounts the authenticated Query boundary; U7 Wishlist commands abort/discard on provider replacement and re-check scope before every projection; U8 Search forwards cancellation; U9 Detail reservation/coupon/recently-viewed and Review create/upload/publication recheck exact route entry plus captured session and abort on disposal. If an already-started reservation or review create is interrupted, local busy state clears but an uncertain terminal prevents a blind second POST. The legacy payment generation blocks a late marker write. | U10/U12 reducers own payment/editor operation IDs; U13 moves remaining profile/reservation reads and commands. |
