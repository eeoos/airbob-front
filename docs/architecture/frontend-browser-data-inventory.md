...
> Baseline: U6 routing followed by the U5 session, U19 structural UI, and U7 auth/wishlist cutovers; recorded 2026-08-29 KST.
...
| TanStack Query cache | session user, search/detail/wishlist/profile/reservation/review/API results | In-memory server-state cache; reload can refetch | Public through Sensitive depending on query | `SessionProvider` owns one QueryClient per subject/epoch generation; Wishlist owns scoped keys/meta/projection, while unmigrated feature hooks retain their current resources | Provider-generation lifetime; default query/mutation meta and Wishlist keys/meta contain current subject/epoch; other legacy keys are not all scoped yet | Every identity boundary cancels and clears the previous client and creates a new one. Wishlist projection predicates also require matching subject/epoch meta before updating owned caches; the temporary legacy Search/Detail projection touches only current-generation query roots. | U5 is the physical bridge and Wishlist completes per-feature scoping U7. U8-U13 migrate remaining resources; U22 removes rollback Query compatibility. Query data is never serialized to browser storage. |
| React component/form and overlay state | login/signup form, search drafts, editor form/images, modal-open/message state, memory-only overlay stack entries and focus targets | Active interaction only; reload is not required | Internal through Personal in owning forms; overlay metadata contains DOM references, opaque registration IDs, and in-memory close callbacks and is never serialized | Owned Auth/Wishlist controllers and remaining feature hooks own domain-local interaction state; `OverlayProvider` owns stack order, close commands, and focus restoration targets | Render/provider lifetime | React unmount clears forms and overlay registrations. U19 removes portal children and Toast timers on unmount and restores body overflow. U7 Wishlist modal close clears only local UI and issues no network request; resumable intent remains workflow-owned. | Keep form/domain state local and overlay coordination app-owned/memory-only. Payment/editor long transactions move to reducers; form data is never persisted by default. |
...
U8-U13 switch the remaining legacy parsers/controllers; those later workflow
...
| --- | --- | --- |
| Login or `/me` refresh to a different viewer | U5 enters identity-change checking, advances epoch, clears checkout/index/payment marker/in-flight registry, cancels and clears the old QueryClient, remounts the authenticated boundary, and only then publishes B in a new client generation. U7 Wishlist commands also abort/discard their generation-local registries and reject stale publication. | U8-U13 finish subject/epoch key/meta and captured-command fences for the remaining features. |
...
| Route/session change during wishlist/review/editor/payment | U5 remounts the authenticated Query boundary; U7 Wishlist commands abort/discard on provider replacement and re-check scope before every projection; review submission re-checks its captured scope; the legacy payment generation blocks a late marker write. Other legacy feature commands still rely on current local refs. | U8-U13 add captured subject/epoch command fences to remaining slices; U9 moves the early review guard into the owned workflow, while U10/U12 reducers own payment/editor operation IDs. |
