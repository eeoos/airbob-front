...
| Toss Payments | `src/platform/integrations/tossPaymentsV1.ts`, adapted by `src/workflows/booking-payment/checkout/paymentGateway.ts` | Temporary CDN v1 gateway behind `PaymentGatewayPort`; the workflow adapter owns current user-facing error policy. U11 replaces only this adapter with npm v2. |
